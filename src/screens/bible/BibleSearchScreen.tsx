...
export default function BibleSearchScreen() {
  ...
  const isLoading = isApiSearch
    ? apiBibleSearch.isLoading
    : segregatedSearch.isLoading;
  ...
  return (
    <SafeAreaScreen>
      ...
      <View className="px-4 pt-2">
        <SearchBar onSearch={setQuery} placeholder="Search verses, topics..." />
      </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text className="text-base text-textPrimary" numberOfLines={3}>
                {item.text}
              </Text>
            </Pressable>
          )}
