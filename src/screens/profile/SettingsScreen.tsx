...
interface MenuItem {
  ...
  label: string;
  screen?: 'EditProfile' | 'LanguageSettings' | 'BlockedUsers' | 'Purchases' | 'Downloads' | 'Bookmarks' | 'Notes';
  onPress?: () => void;
  destructive?: boolean;
  rightLabel?: string;
}
...
export default function SettingsScreen() {
  ...
  const handleLogout = () => {
    confirmAction(t('settings.logout'), t('settings.logoutConfirm'), () => logout(), t('settings.logout'));
  };

  const handleDeleteAccount = () => {
    confirmAction(t('settings.deleteAccount'), t('settings.deleteAccountConfirm'), () => {
    ...
    <SafeAreaScreen>
      ...
      <ScrollView className="flex-1 px-4 pt-2">
        {menuItems.map((item, index) => (
          ...
          >
            <Ionicons name={item.icon} size={22} color={item.destructive ? colors.error : colors.primary.DEFAULT} />
            ...
            {!item.destructive && <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} />}
          </AnimatedPressable>
        ))}
