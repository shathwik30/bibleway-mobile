...
export default function CommentsScreen() {
  ...
  const allComments = flattenPages(commentsQuery.data);
  ...
  const inputPlaceholder = replyingToCommentId
    ? "Write a reply..."
    : "Write a comment...";
  ...
  const renderComment = ({ item }: { item: Comment }) => (
    <View>
      ...
      />
      {expandedCommentId === item.id && <RepliesSection commentId={item.id} />}
    </View>
  );
...
const RepliesSection = React.memo(function RepliesSection({
  commentId,
}: {
  commentId: string;
}) {
  const repliesQuery = useReplies(commentId);
  const allReplies = flattenPages(repliesQuery.data);
